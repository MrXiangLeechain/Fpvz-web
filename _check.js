...
const PLANTS={
  sunflower :{name:"向日葵",  emoji:"🌻", cost:50,  hp:80,  cd:0.85, desc:"定期产出阳光"},
  peashooter:{name:"豌豆射手",emoji:"🌱", cost:100, hp:100, cd:1,    dmg:20, rate:1.4, desc:"直线发射豌豆"},
  wallnut   :{name:"坚果墙",  emoji:"🥜", cost:50,  hp:400, cd:3.5,  desc:"高血量肉盾，挡住僵尸"},
  snowpea   :{name:"寒冰射手",emoji:"🌱", cost:175, hp:100, cd:1.25, dmg:20, rate:1.4, snow:true, badge:"❄", desc:"冰豌豆，减速僵尸"},
...
/* 刷怪模型：
   - 持续“涓流”刷怪：前段每次 1~2 只（不同行），后段每次 3~4 只（最多 4 只同时），高级僵尸过半后增多
   - “旗波” waves：特定时刻成批涌出（原版“一大波”的来源），每批最多 4 只同时、分批延后到场
   - dur 为每关总时长（不变）；进度条按 dur 计算 */
const LEVELS=[
  {name:"第 1 关 · 后院初见", plants:["sunflower","peashooter"], startSun:150, skySun:true, dur:60,
   ztypes:["normal"], trickle:{gapFront:9, gapBack:4.5, frontMax:2, backMax:4},
   waves:[{t:30,z:{normal:2}},{t:60,z:{normal:4},final:true}]},
  {name:"第 2 关 · 路障来袭", plants:["sunflower","peashooter","wallnut"], startSun:100, skySun:true, dur:78,
   ztypes:["normal","cone"], trickle:{gapFront:9, gapBack:4.5, frontMax:2, backMax:4},
   waves:[{t:38,z:{normal:2,cone:1}},{t:78,z:{cone:3,normal:4},final:true}]},
  {name:"第 3 关 · 铁桶压境", plants:["sunflower","peashooter","wallnut","snowpea"], startSun:100, skySun:true, dur:86,
   ztypes:["normal","cone","bucket"], trickle:{gapFront:8.5, gapBack:4.2, frontMax:2, backMax:4},
   waves:[{t:42,z:{cone:2,normal:2}},{t:86,z:{bucket:3,cone:3,normal:4},final:true}]},
  {name:"第 4 关 · 疾影夜袭", plants:["sunflower","peashooter","wallnut","snowpea","cherry"], startSun:125, skySun:true, dur:82,
   ztypes:["normal","cone","bucket","runner"], trickle:{gapFront:8, gapBack:4, frontMax:2, backMax:4},
   waves:[{t:40,z:{cone:2,runner:2}},{t:82,z:{bucket:3,cone:4,runner:4},final:true}]},
  {name:"第 5 关 · 决战黎明", plants:["sunflower","peashooter","wallnut","snowpea","cherry","repeater"], startSun:125, skySun:true, dur:96,
   ztypes:["normal","cone","bucket","runner"], trickle:{gapFront:7.5, gapBack:3.8, frontMax:2, backMax:4},
   waves:[{t:48,z:{normal:2,cone:3,runner:2}},{t:96,z:{bucket:4,cone:4,runner:5},final:true}]},
];

/* ================= 存档 ================= */
const SAVE_KEY="pvzWebProgress";
const MUTE_KEY="pvzWebMuted";
...
/* 单音：freq 起始频率，slideTo 终止频率（做滑音），delay 延迟播放 */
function tone(freq,dur,type,vol,slideTo,delay){
  const ac=audioInit();
  if(!ac || G.muted || !freq) return;
  try{
...
/* 高频音效节流：豌豆/啃食/命中每秒可能发生十几次，不节流会糊成噪音 */
const sfxLast={};
function sfxGap(key,ms){
  const t=performance.now();
  if(sfxLast[key] && t-sfxLast[key]<ms) return false;
  sfxLast[key]=t; return true;
}
const SFX={
  plant (){ tone(430,0.13,"triangle",0.17,720); },
  sun   (){ tone(880,0.10,"sine",0.13,1320); tone(1320,0.08,"sine",0.07,null,0.045); },
  shoot (){ if(sfxGap("shoot",90))  tone(240,0.05,"square",0.045,150); },
  hit   (){ if(sfxGap("hit",70))    tone(170,0.05,"square",0.06,120); },
  eat   (){ if(sfxGap("eat",420))   tone(130,0.07,"sawtooth",0.05,95); },
  die   (){ if(sfxGap("die",90)){ tone(210,0.30,"sawtooth",0.12,70); noiseBurst(0.16,0.08,700); } },
  boom  (){ noiseBurst(0.5,0.32,1000); tone(95,0.42,"sawtooth",0.20,38); },
  wave  (){ [0,0.22,0.44].forEach((d,i)=>tone(330+i*60,0.18,"square",0.10,null,d)); },
  win   (){ [523,659,784,1047].forEach((f,i)=>tone(f,0.20,"triangle",0.15,null,i*0.13)); },
  lose  (){ [392,330,262,196].forEach((f,i)=>tone(f,0.30,"sawtooth",0.14,null,i*0.17)); },
  click (){ if(sfxGap("click",40))  tone(700,0.045,"sine",0.07); },
  mow   (){ tone(160,0.35,"square",0.10,600); },
};
function refreshMuteBtn(){
  const b=$("mute-btn");
  if(!b) return;
  b.textContent=G.muted?"🔇":"🔊";
  b.classList.toggle("sel",G.muted);
}
function toggleMute(){
  G.muted=!G.muted; saveMuted(G.muted);
  refreshMuteBtn();
  if(!G.muted){ audioResume(); SFX.click(); }
}

function resetLevelState(){
  G.grid=[]; G.zombies=[]; G.peas=[]; G.suns=[]; G.fx=[];
  G.waveIdx=0; G.allSpawned=false; G.time=0; G.selected=null; G.shovel=false;
  G.spawnTimer=6; G.spawnQueue=[];
  G.skySunTimer=5;
...
function spawnZombie(type,row){
  if(row===undefined) row=Math.floor(Math.random()*ROWS);
...
/* 取 n 个互不相同的行（保证一批僵尸不会挤在同一行） */
function pickDistinctRows(n){
  const all=[...Array(ROWS).keys()];
  for(let i=all.length-1;i>0;i--){ const j=Math.floor(Math.random()*(i+1)); [all[i],all[j]]=[all[j],all[i]]; }
  return all.slice(0,n);
}
/* 按关卡可用僵尸 + 是否过半，加权挑选类型（过半后高级僵尸更多） */
function pickType(lv,front){
  const t=lv.ztypes;
  const base={normal:1,cone:1,bucket:1,runner:1};
  let total=0; const cand=[];
  t.forEach(ty=>{
    let wt=base[ty]||1;
    if(front){
      if(ty==="normal") wt*=3; else if(ty==="bucket") wt*=0.5; else if(ty==="runner") wt*=0.8;
    }else{
      if(ty==="cone") wt*=2; else if(ty==="bucket") wt*=2.5;
      else if(ty==="runner") wt*=2.2; else if(ty==="normal") wt*=0.5;
    }
    cand.push({ty,wt}); total+=wt;
  });
  let r=Math.random()*total;
  for(const c of cand){ if(r<c.wt) return c.ty; r-=c.wt; }
  return t[0];
}
/* 旗波：总量按每批 4 只拆分，第 1 批立即到场、其余每批延后 1 秒（最多 4 只同时出现） */
function spawnFlagWave(w){
  const list=[];
  Object.entries(w.z).forEach(([t,n])=>{ for(let i=0;i<n;i++) list.push(t); });
  for(let i=list.length-1;i>0;i--){ const j=Math.floor(Math.random()*(i+1)); [list[i],list[j]]=[list[j],list[i]]; }
  let batch=0;
  for(let i=0;i<list.length;i+=4){
    const chunk=list.slice(i,i+4);
    const rows=pickDistinctRows(chunk.length);
    chunk.forEach((ty,k)=>{
      if(batch===0) spawnZombie(ty,rows[k]);
      else G.spawnQueue.push({type:ty,row:rows[k],at:G.time+batch});
    });
    batch++;
  }
}
/* 涓流：前段 1~2 只、后段 3~4 只，均分布在不同行 */
function trickleBatch(lv){
  const dur=lv.dur, front=G.time<dur/2;
  const maxN=front?lv.trickle.frontMax:lv.trickle.backMax;
  let n=front ? (Math.random()<0.3?2:1) : (Math.random()<0.5?4:3);
  n=Math.min(n,maxN);
  const rows=pickDistinctRows(n);
  for(let k=0;k<n;k++) spawnZombie(pickType(lv,front),rows[k]);
}
...
function update(dt){
  ...
  while(G.waveIdx<lv.waves.length && G.time>=lv.waves[G.waveIdx].t){
    spawnFlagWave(lv.waves[G.waveIdx]);
    if(lv.waves[G.waveIdx].final){ flashText("🚩 最后一大波僵尸来袭！",W/2,200,"#ff7b6b",30,3); SFX.wave(); }
    G.waveIdx++;
  }
  /* 涓流：在总时长内按节奏刷出小股僵尸 */
  if(G.time<dur){
    ...
    if(G.spawnTimer<=0){
      trickleBatch(lv);
      const front=G.time<dur/2;
      G.spawnTimer=(front?lv.trickle.gapFront:lv.trickle.gapBack)*(0.8+Math.random()*0.4);
    }
  }
  /* 旗波分批延后到场的僵尸 */
  if(G.spawnQueue.length){
    G.spawnQueue=G.spawnQueue.filter(s=>{
      if(G.time>=s.at){ spawnZombie(s.type,s.row); return false; }
      return true;
    });
  }
  /* 全部刷完判定：到达总时长且队列清空 */
  if(G.time>=dur && G.spawnQueue.length===0) G.allSpawned=true;
  ...
  for(let r=0;r<ROWS;r++) for(let c=0;c<COLS;c++){
    ...
    if(pl.type==="sunflower"){
      pl.timer+=dt;
      if(pl.timer>=12){ pl.timer=0; spawnSun(cellCX(c)+28,cellCY(r)-20,cellCY(r)+18,true); }
    }else if(def.dmg){
      pl.fireT-=dt;
      ...
      if(pl.shot2>0){ pl.shot2-=dt; if(pl.shot2<=0){ G.peas.push({row:r,x:cellCX(c)+22,y:cellCY(r)-14,snow:!!def.snow,vx:420,dmg:def.dmg}); SFX.shoot(); } }
    }else if(def.bomb){
      pl.fuse-=dt;
  ...
  for(const z of G.zombies){
    ...
    if(z.x<GRID_X+4){
      ...
      if(m&&m.state==="idle"){ m.state="run"; SFX.mow(); }
...
function render(){
  /* 全屏清屏（含草坪上方的条带，否则天降阳光会留下拖影） */
  ctx.fillStyle="#2f4425"; ctx.fillRect(0,0,W,H);
  /* 草坪 */
  for(let r=0;r<ROWS;r++) for(let c=0;c<COLS;c++){
  ...
  /* 房子车道（最左侧专属区域）——整段 save/restore，防止半透明 fillStyle 外泄 */
  ctx.save();
  ...
  for(let r=0;r<ROWS;r++) for(let c=0;c<COLS;c++){
    ...
    ctx.fillStyle="#000";          /* emoji 用自身颜色，避免沿用任何半透明 fillStyle */
  ...
  G.zombies.forEach(z=>{
    ...
    drawHat(z);
    if(z.eat){
      ctx.font="14px serif"; ctx.fillStyle="#000"; ctx.fillText("😋",z.x-20,y-30);
    }
    ctx.restore();
  });

  /* 僵尸血条（头顶）：帽子护甲段用金属灰区分，体现"先掉帽子再掉血" */
  G.zombies.forEach(z=>{
    const def=ZOMBIES[z.type];
    const bw=46, bh=5;
    const bx=z.x-bw/2, by=cellCY(z.row)-48;
    const frac=Math.max(0,Math.min(1,z.hp/z.maxHp));
    ctx.fillStyle="rgba(0,0,0,.55)"; ctx.fillRect(bx-2,by-2,bw+4,bh+4);   // 描边
    ctx.fillStyle="#4a4a4a";         ctx.fillRect(bx,by,bw,bh);           // 空槽
    ctx.fillStyle=frac>.5?"#7ed321":frac>.25?"#f5a623":"#e74c3c";         // 本体血
    ctx.fillRect(bx,by,bw*frac,bh);
    if(def.hatHP){
      const hatStart=Math.max(0,1-def.hatHP/z.maxHp);
      if(frac>hatStart){
        ctx.fillStyle="#b8c4cc";
        ctx.fillRect(bx+bw*hatStart,by,bw*(frac-hatStart),bh);
      }
    }
  });
  ...
  /* 波次进度条（仅右下角，长度 = 原长 1/3，按总时长 dur 计算） */
  if(G.scene==="play"){
    const lv=levelDef();
    const dur=lv.dur;
    const fullW=W-GRID_X-30;
    const bw=fullW/3;
    const bx=W-12-bw, by=H-14;
    ctx.fillStyle="rgba(0,0,0,.45)"; ctx.fillRect(bx-4,by-5,bw+8,10);
    ctx.fillStyle="#5d8a3c"; ctx.fillRect(bx,by-3,bw,6);
    ctx.fillStyle="#ffd94d"; ctx.fillRect(bx,by-3,bw*Math.min(1,G.time/dur),6);
    lv.waves.forEach(w=>{
      const fx2=bx+bw*(w.t/dur);
      ctx.fillStyle=w.final?"#e74c3c":"#3f6f2a";
      ctx.fillRect(fx2-2,by-8,4,16);
    });
    ctx.fillStyle="rgba(255,255,255,.85)"; ctx.font="11px sans-serif"; ctx.textAlign="right";
    ctx.fillText(`第 ${Math.min(G.waveIdx+(G.allSpawned?0:1),lv.waves.length)} / ${lv.waves.length} 波`,bx+bw,by-14);
  }
...
function onCanvasClick(e){
  ...
  /* 1. 收阳光 */
  for(const s of G.suns){
    if(!s.fly && Math.hypot(s.x-pos.x,s.y-pos.y)<34){ s.fly=true; SFX.sun(); return; }
  }
...
document.addEventListener("keydown",e=>{
  if(e.key==="m"||e.key==="M"){ toggleMute(); return; }   /* 静音开关在任何场景都可用 */
...
/* pointerdown 延迟兜底：某些嵌入/预览环境会吞掉 click。
   做法：pointerdown 先登记，若 250ms 内没有 click 到达，说明 click 被吞，再补触发一次。
   正常环境下 click 会先到并取消兜底，因此不会双触发，也不影响快速连点。 */
let pendingPtr=null;
const CLICKABLE=".seed,.level-card,#btn-start,#btn-resume,#btn-restart,#btn-quit,#btn-next,#btn-retry,#btn-menu2,#btn-back-menu,#shovel-btn,#pause-btn";
document.addEventListener("pointerdown",e=>{
  const target=e.target;
  if(e.button!==0 || !target || !target.closest) return;
  const onCvs=!!target.closest("canvas#game");
  const clickable=onCvs?null:target.closest(CLICKABLE);
  if(!onCvs && !clickable) return;
  if(pendingPtr) clearTimeout(pendingPtr.timer);
  const x=e.clientX, y=e.clientY;
  pendingPtr={
    timer:setTimeout(function(){
      pendingPtr=null;
      if(onCvs) onCanvasClick({clientX:x, clientY:y});
      else clickable.click();
    },250)
  };
},true);
document.addEventListener("click",function(){
  if(pendingPtr){ clearTimeout(pendingPtr.timer); pendingPtr=null; }
},true);
...
buildLevelGrid();
refreshMuteBtn();         // 恢复上次的音效开关状态
/* 浏览器自动播放策略：AudioContext 必须在首次用户交互后才能出声 */
["pointerdown","keydown","touchstart"].forEach(ev=>document.addEventListener(ev,audioResume));
/* 按钮点击音：统一在捕获阶段处理，不必逐个按钮改 */
document.addEventListener("click",e=>{
  audioResume();
  const t=e.target;
  if(t && t.closest && t.closest(".tool-btn,.btn")) SFX.click();
},true);
showOverlay("ov-menu");
requestAnimationFrame(loop);
